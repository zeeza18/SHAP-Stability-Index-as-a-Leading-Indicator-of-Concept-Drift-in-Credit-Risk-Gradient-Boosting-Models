"""Adaptive LightGBM model — retrains when ADWIN detects drift."""
...
class AdaptiveLightGBM(BaseModel):
    """LightGBM with ADWIN-triggered adaptive retraining."""
    ...
    def __init__(
        ...
        super().__init__(name="AdaptiveLightGBM", random_state=random_state)
    ...
    def fit(self, X: pd.DataFrame, y: pd.Series, **kwargs) -> "AdaptiveLightGBM":
        self._model.fit(X, y)
        return self
    ...
    def update_window(
        ...
        if drift_detected and len(self._buffer) > 0:
            ...
            print(
                f"[AdaptiveLightGBM] Drift @ window {window_index}. "
    ...
    def load(self, path: Path) -> "AdaptiveLightGBM":
